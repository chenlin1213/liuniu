    ...
    public static function getModelTime($where, $model = null, $prefix = 'add_time', $data = 'data', $field = ' - ')
    {
        ...
        switch ($where[$data]) {
            ...
            case 'quarter':
                ...
                $model = $model->where($prefix, 'between', [strtotime($startTime), strtotime($endTime)]);
//                $model = $model->where($prefix, '<', strtotime($endTime));
            ...
            default:
                if (strstr($where[$data], $field) !== false) {
                    ...
                    $model = $model->where($prefix, 'between', [strtotime($startTime), bcadd(strtotime($endTime), 86400, 0)]);
//                    $model = $model->where($prefix, '<',);
                }
                break;
        }
        return $model;
