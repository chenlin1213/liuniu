...
class StoreProductController
{
            ...
            switch ($userType) {
                ...
                case 'routine':
                    ...
                    if (!$imageInfo) {
                        ...
                        $res = \app\models\routine\RoutineCode::getPageCode('pages/product/product', $data, 280);
    ...
    {
        list($bargainId, $from) = UtilService::postMore([['productId', 0], ['from', 'wechat']], $request, true);
...
//        var_dump($user);
        ...
        try {
            $siteUrl = sys_config('site_url');
            $data['title'] = $storeBargainInfo['store_name'];
            $data['image'] = $storeBargainInfo['image'];
            $data['price'] = $price;
            $data['user_nickname'] = $user['nickname'];
            $data['user_avatar'] = $user['avatar'];
            $pinyin = new Pinyin();
            $data['user_nickname_pinyin'] = $pinyin->convert($user['nickname']);
            $data['ot_price'] = $storeBargainInfo['ot_price'];
            if ($from == 'routine') {
                //小程序
                $name = $bargainId . '_' . $user['uid'] . '_' . $user['is_promoter'] . '_product_share_routine.jpg';
                $imageInfo = SystemAttachment::getInfo($name, 'name');
                if (!$imageInfo) {
                    $valueData = 'id=' . $bargainId;
                    if ($user['is_promoter'] || sys_config('store_brokerage_statu') == 2) $valueData .= '&pid=' . $user['uid'];
                    $res = RoutineCode::getPageCode('pages/product/product', $valueData, 280);
                    if (!$res) return app('json')->fail('二维码生成失败');
                    $uploadType = (int)sys_config('upload_type', 1);
                    $upload = new Upload($uploadType, [
                        'accessKey' => sys_config('accessKey'),
                    ...
                    $res = $upload->to('routine/product/code')->validate()->stream($res, $name);
                    if ($res === false) {
                        return app('json')->fail($upload->getError());
                    }
                    $imageInfo = $upload->getUploadInfo();
                ...
                $posterImage = UtilService::setShareProductPoster($data, 'routine/product/poster');
                ...
                //公众号
                $name = $bargainId . '_' . $user['uid'] . '_' . $user['is_promoter'] . '_product_share_wap.jpg';
                $imageInfo = SystemAttachment::getInfo($name, 'name');
                if (!$imageInfo) {
                    $codeUrl = UtilService::setHttpType($siteUrl . '/index/pages/product/product?id=' . $bargainId . '&spread=' . $user['uid'], 0);//二维码链接
                ...
                $posterImage = UtilService::setShareProductPoster($data, 'wap/product/poster');
                if (!is_array($posterImage)) return app('json')->fail('海报生成失败');
                SystemAttachment::attachmentAdd($posterImage['name'], $posterImage['size'], $posterImage['type'], $posterImage['dir'], $posterImage['thumb_path'], 1, $posterImage['image_type'], $posterImage['time'], 2);
                if ($posterImage['image_type'] == 1) $posterImage['dir'] = $siteUrl . $posterImage['dir'];
