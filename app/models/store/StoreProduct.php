...
//        if($salesOrder) $baseOrder = $salesOrder == 'desc' ? 'sales DESC' : 'sales ASC';//真实销量
    ...
    public static function getProductBrokerage(array $cartId, bool $type = true)
    {
            ...
            if (isset($product['productInfo'])) {
                ...
                if (isset($productInfo['is_sub']) && $productInfo['is_sub'] == 1) {
                    if ($productInfo['store_type'] == 1) {
                        $oneBrokerageReal = bcadd($oneBrokerageReal, bcmul($cartNum, $productInfo['attrInfo']['brokerage'] ?? 0, 2), 2);
                        $twoBrokerageReal = bcadd($twoBrokerageReal, bcmul($cartNum, $productInfo['attrInfo']['brokerage_two'] ?? 0, 2), 2);
                    } else {
        ...
        if ($type) {
            ...
            $brokeragePriceReal = bcmul($sumProductPriceReal, $brokerageRatio, 2);
            $brokeragePriceVirtual = bcmul($sumProductPriceVirtual, $brokerageRatio, 2);
            //固定返佣 + 比例返佣 = 一级总返佣金额
            return [bcadd($oneBrokerageReal, $brokeragePriceReal, 2), bcadd($oneBrokerageVirtual, $brokeragePriceVirtual, 2)];
        } else {
            //获取二级返佣比例
            $storeBrokerageTwo = sys_config('store_brokerage_two');
            //二级返佣比例小于等于0 直接返回
            if ($storeBrokerageTwo <= 0) {
                return [$twoBrokerageReal, $twoBrokerageVirtual];
            }
            //计算获取二级返佣比例
            ...
            $brokeragePriceVirtual = bcmul($sumProductPriceVirtual, $brokerageRatio, 2);
            //固定返佣 + 比例返佣 = 二级总返佣金额
            return [bcadd($twoBrokerageReal, $brokeragePriceReal, 2), bcadd($twoBrokerageVirtual, $brokeragePriceVirtual, 2)];
        }
